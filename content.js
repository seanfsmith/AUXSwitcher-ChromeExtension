...
chrome.runtime.sendMessage({type: "getToToggle"}, function(response) {
    updateList();
    auxSwitcher(response.aux,minToMS(response.time));
});
...
function auxSwitcher(aux,time){
    var activeAux = getActiveAux();
    var index = lists.indexOf(aux);
